...
extern char send_command_neil[2000];
...
void DOS_Shell::Run(void) {
	...
	do {
		...
		//don't parse our commands if we are running a batch file
		int  len = (int)strlen(send_command_neil);
		if (bf == NULL && len > 0)
		{
			//split by \n
			std::vector<std::string> all_commands =
				string_split(std::string(send_command_neil), std::string("\n"));

			sprintf(send_command_neil, "");

			for (auto command : all_commands)
			{
				sprintf(input_line, command.c_str());
